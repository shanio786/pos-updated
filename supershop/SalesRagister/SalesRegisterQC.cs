...
namespace supershop 
{
    public partial class SalesRegisterQC : Form
    {
            ...
            try
            {
              ...
              //  DataAccess.ExecuteSQL(sqlVatdis);
               ...
               //  dgrvSalesItemList.ReadOnly = true;
                ...
                dgrvSalesItemList.Columns["Del"].Width = 35;
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        //int y = dgv_items.Rows.Add();
                        //dgv_items.Rows[y].Cells[0].Value = dr["product_id"].ToString();
                        //if (dr["taxapply"].ToString() == "1")
                        //{
                        //    dgv_items.Rows[y].Cells[1].Value = "YES";
                        //}
                        //else
                        //{
                        //    dgv_items.Rows[y].Cells[1].Value = "NO";
                        //}

                        //dgv_items.Rows[y].Cells[2].Value = dr["product_name"].ToString();
                        //dgv_items.Rows[y].Cells[3].Value = dr["product_quantity"].ToString();
                        //dgv_items.Rows[y].Cells[4].Value = dr["retail_price"].ToString();
                        //dgv_items.Rows[y].Cells[5].Value = dr["discount"].ToString();
                        //dgv_items.Rows[y].Cells[6].Value = dr["category"].ToString();
                        //dgv_items.Rows[y].Cells[7].Value = dr["supplier"].ToString();
        ...
        private void txtBarcodeReaderBox_TextChanged(object sender, EventArgs e)        
        {
            ...
            if (txtBarcodeReaderBox.Text == "")
            {
                AutoClosingMessageBox.Show("Please insert product id", "Caption", 10);
                //textBox1.Focus();
                txtBarcodeReaderBox.Focus();
            }
            else
            {
                 try
                {
                    ...
                    if (supershop.BarCode.WeightBarcode.TryAdd(txtBarcodeReaderBox.Text, dgrvSalesItemList, Taxrate))
                    {
                        ...
                        txtBarcodeReaderBox.Text = "";
                        txtBarcodeReaderBox.Focus();
                        btnSuspend.Enabled = true; btnPayment.Enabled = true; btnCompleteSalesAndPrint.Enabled = true; btnSaveOnly.Enabled = true;
                        DiscountCalculation();
                        vatcal();
                        return;
                    }
                    ...
                    DataTable dt = DataAccess.GetDataTable(sql, DataAccess.P("@taxrate", Taxrate), DataAccess.P("@wholesale", chkWholesale.Checked ? 1 : 0), DataAccess.P("@id", txtBarcodeReaderBox.Text));
        ...
        private static double ToDouble(object value)
        {
            ...
            return t.Length == 0 ? 0 : Convert.ToDouble(t);
        }
        ...
        public long SaveSale(string payamount, string changeamount, string dueamount, string salesdate, string Comment, System.Collections.Generic.List<SalesRagister.Tender> tenders)
        {
            ...
            DataAccess.RunInTransaction(delegate(DataAccess.DbTransaction tx)
            {
                ...
                for (int i = 0; i < rows; i++)
                {
                    ...
                    double Profit = Math.Round(((Rprice - (Rprice * discount) / 100.00) - cost_price), 2);
        ...
        private void btnPayment_Click(object sender, EventArgs e)
        {
            ...
            //var lpa = lblTotalPayable.Text;

            // Code Comment at February 06, 2021
            ...
            //Code Comment at February 07, 2021
        ...
        private void btnResume_Click(object sender, EventArgs e)
        {
            try
            {
                using (SalesRagister.HeldSalesForm f = new SalesRagister.HeldSalesForm())
                {
                    ...
                    SalesRagister.HeldSaleStore.Delete(holdId);
                    tabPageSR_Counter.Text = "Terminal";
                    LoadTotalDiscount();
                    LoadTotalTax();
                    DiscountCalculation();
            ...
            catch (Exception ex) { Logger.Show(ex, "Could not resume the held sale."); }
        }
        ...
        {
            string sql = "SELECT product_quantity FROM purchase where product_id = @id";
            DataTable dt = DataAccess.GetDataTable(sql, DataAccess.P("@id", itemcode));
        ...
        void printDocument_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            string sql = " SELECT  sp.sales_id AS salesid, sp.payment_type AS paytype, sp.payment_amount AS Payamount, " +
                            ...
                            " ON  sp.c_id  = c.ID " +
                            " Where sp.sales_id  = @inv ";
            DataTable dt = DataAccess.GetDataTable(sql, DataAccess.P("@inv", txtInvoice.Text));
        ...
        private void dataGridView1_KeyDown(object sender, KeyEventArgs e)
        {
            if(e.KeyCode==Keys.Enter)
            {
                ...
                txtBarcodeReaderBox.Text = dgrvSalesItemList.SelectedRows[0].Cells[0].Value.ToString();
                SetDiscount();
                dgrvSalesItemList.Hide();
                txtPaidAmount.Focus();
            }
        }
